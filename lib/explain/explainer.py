...
from lib.explain import (
    ExplainerAlgorithm,
    Explanation,
    HeteroExplanation,
)
from lib.explain.algorithm.utils import (
    clear_masks,
    set_hetero_masks,
    set_masks,
)
...
class Explainer:
    ...
    Args:
        ...
        model_config (ModelConfig): The model configuration.
            See :class:`~lib.explain.config.ModelConfig` for
        ...
        threshold_config (ThresholdConfig, optional): The threshold
            configuration.
            See :class:`~lib.explain.config.ThresholdConfig` for
            available options. (default: :obj:`None`)
    """
    def __init__(
        self,
    ...
    @torch.no_grad()
    def get_prediction(self, x, edge_index,batch,edge_attr, *args, **kwargs) -> Tensor:
        ...
        # with torch.no_grad():
        out = self.model( x, edge_index,batch=batch, edge_attr=edge_attr,*args, **kwargs)
    ...
    def get_masked_prediction(
        ...
        **kwargs,
        ...
        out = self.get_prediction(x, edge_index, batch=batch, edge_attr=edge_attr,**kwargs)
        ...
        if self.explanation_type == ExplanationType.phenomenon:
            ...
            if target is not None:
                warnings.warn(
                    ...
                    f"type '{self.explanation_type.value}'")
            # prediction = self.get_prediction(x, edge_index, batch=batch, edge_attr=edge_attr,**kwargs)
            prediction = self.get_prediction(x, edge_index, batch=batch, edge_attr=edge_attr, **kwargs)
