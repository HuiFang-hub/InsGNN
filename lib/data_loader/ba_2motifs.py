...
import dgl
def read_ba2motif_data(folder: str, prefix,cfg, multi_label,l_min,shapelet):
    with open(os.path.join(folder, f"{prefix}.pkl"), 'rb') as f:
        dense_edges, node_features, graph_labels = pickle.load(f)
    f = []
    tmp = graph_labels[:,1].astype(int)
    num_class = len(np.bincount(tmp))
    data_list = []
    pbar = tqdm(range(dense_edges.shape[0]))
    for graph_idx in pbar:
        ...
        if shapelet:
            ...
            feature = KVs_staticG(cfg, x, node_label, num_class, multi_label,l_min).cpu()
        ...
        data_list.append(Data(graph=g, x=x, edge_index=edge_index, y=y, node_label=node_label, edge_label=edge_label))
    f = torch.cat(f)
    pickle.dump(f, open(os.path.join(folder, f"{prefix}_shapelets.pkl"), 'wb'))
    return data_list
...
class SynGraphDataset(InMemoryDataset):
    ...
    def __init__(self, root, name,cfg, multi_label,l_min,shapelet=False, transform=None, pre_transform=None):
    ...
    def process(self):
        if self.name.lower() == 'BA_2Motifs'.lower():
            data_list = read_ba2motif_data(self.raw_dir, self.names[self.name][2],self.cfg,self.multi_label,self.l_min,self.shapelet)
