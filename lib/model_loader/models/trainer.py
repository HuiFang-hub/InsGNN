...
from GSAT.utils import process_data, get_preds, save_checkpoint,Criterion
...
def run_one_epoch(dataset_name, data_loader, model, criterion, optimizer, epoch, phase, device,random_state, use_edge_attr, multi_label):
    loader_len = len(data_loader)
...
def update_best_epoch_res(gsat, train_res, valid_res, test_res, metric_dict, dataset_name, epoch, model_dir):
    # assert len(train_res) == 5  #att_auroc, None, clf_acc, clf_roc, avg_loss
    ...
    if (current_r == gsat.final_r) and ((valid_res[main_metric_idx] > metric_dict['best_clf_valid_roc'])
                                        or (valid_res[main_metric_idx] == metric_dict['best_clf_valid_roc'] and valid_res[4] < metric_dict['best_clf_valid_loss'])):

        metric_dict = {'best_clf_epoch': epoch, 'best_clf_valid_loss': valid_res[4],
                       'best_clf_train_roc': train_res[main_metric_idx], 'best_clf_valid_roc': valid_res[main_metric_idx], 'best_clf_test_roc': test_res[main_metric_idx],
                      'best_clf_train_acc': train_res[2], 'best_clf_valid_acc': valid_res[2], 'best_x_test_acc':test_res[2],
                       'best_x_train_roc': train_res[0], 'best_x_valid_roc': valid_res[0], 'best_x_test_roc': test_res[0]}
